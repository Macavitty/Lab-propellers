            ...
            try {
                ...
                switch (command){
                    ...
                    case "aaa":
                        ...
                        helper.append("\t \t" + (char)27 + "[35maddIfMax " + (char)27 + "[30mto add propeller from json file if it`s efficiency is maximal in the collection\n");
                    ...
                    case "tell_me":
                        if (propellerCollection.getPropellerMap().isEmpty()) oos.writeObject("* Hang on! Add at least one propeller.\n  No propeller - no story! *");
                        else try{ story.tellStory(propellerCollection, oos);}catch (Throwable t){/**/}
                        break;

                    case "":
                        break;

                    default: oos.writeObject("* wrong command format *");
                }
